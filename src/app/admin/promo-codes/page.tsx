...
export default async function PromoCodesAdminPage() {
  ...
  // Scope options for the create form - a single flat <select> ("national"
  // / "state:<id>" / "area:<id>") instead of cascading dropdowns, since a
  // plain server-rendered form can't cascade without client JS.
  ...
  if (isNationalAdmin) {
    ...
    scopeOptions = [
      ...
      ...allStates.map((s) => ({ value: `state:${s.id}`, label: `All of ${s.name}` })),
      ...(allAreas ?? []).map((a) => ({
        value: `area:${a.id}`,
        label: `${a.name}, ${stateById.get(a.state_id)?.name ?? ""}`,
      })),
    ];
    ...
    const stateById = new Map((myStates ?? []).map((s) => [s.id, s]));
    scopeOptions = [
      ...(myStates ?? []).map((s) => ({ value: `state:${s.id}`, label: `All of ${s.name}` })),
      ...(myAreas ?? []).map((a) => ({
        value: `area:${a.id}`,
        label: `${a.name}, ${stateById.get(a.state_id)?.name ?? ""}`,
      })),
    ...
    if (accessibleAreaIds.length <= 1) {
      ...
      scopeOptions = areaId ? [{ value: `area:${areaId}`, label: area?.name as string }] : [];
    } else {
      const { data: myAreas } = await supabase
        ...
        .order("name");
      scopeOptions = (myAreas ?? []).map((a) => ({ value: `area:${a.id}`, label: a.name as string }));
    }
  }
    ...
    <div>
      ...
      <section className="mt-8 rounded-2xl border border-slate-200 p-6">
        ...
        <form action={createPromoCode} className="mt-4 flex flex-wrap items-end gap-3">
          ...
          {fixedAreaLabel ? (
            <>
              <input type="hidden" name="scope" value={scopeOptions[0]?.value ?? ""} />
