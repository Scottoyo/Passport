  ...
  if (mode === "edit") {
    ...
    return (
      <div className="mx-auto max-w-2xl">
        <Link href={baseHref} className="text-sm font-semibold text-ink-muted hover:text-ink">
          &larr; {lead.business_name}
        </Link>
        <h1 className="mt-2 text-2xl font-bold text-ink">Edit Lead</h1>
        <div className="mt-8">
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-ink">{lead.business_name}</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`${baseHref}?mode=edit`} className={buttonClasses("outline")}>
            Edit
          </Link>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-border bg-surface p-6">
          <h2 className="font-semibold text-ink">Lead details</h2>
          <dl className="mt-3 space-y-3 text-sm">
            <div>
              <dt className="text-ink-muted">Disposition</dt>
              <dd className="text-ink">{DISPOSITION_LABELS[lead.disposition]}</dd>
            </div>
            <div>
              <dt className="text-ink-muted">Contact</dt>
              <dd className="text-ink">{contactName || "-"}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="text-ink">{new Date(lead.updated_at).toLocaleString()}</dd>
            </div>
          </dl>
