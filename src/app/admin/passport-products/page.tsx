...
import type { PassportProduct } from "@/lib/types/domain";
...
export default async function PassportProductsAdminPage() {
  ...
  const productsByState = new Map<string, PassportProduct[]>();
  for (const p of products) {
    const list = productsByState.get(p.state_id) ?? [];
    list.push(p);
    productsByState.set(p.state_id, list);
  }
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-600">
        Each state has its own pricing — a Passport bought for one state
        only works in that state.
      </p>
        ...
        {states.map((state) => (
          <div key={state.id} className="rounded-2xl border border-slate-200 p-6">
            ...
            <div className="mt-4 space-y-4">
              {(productsByState.get(state.id) ?? []).map((p) => (
              ...
              {(productsByState.get(state.id) ?? []).length === 0 && (
                <p className="text-sm text-slate-500">No products for {state.name} yet.</p>
              )}
            </div>
