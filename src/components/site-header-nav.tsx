...
import { NotificationBell, type NotificationBellEvent } from "@/components/notification-bell";
import { AdminNotificationBell, type AdminNotificationBellItem } from "@/components/admin/admin-notification-bell";
...
// client-side navigation - rather than from a server-computed prop from the
// root layout. The root layout's own region resolution only re-runs when
// Next.js actually re-executes that layout, which it does NOT do on every
// client-side Link navigation (shared layouts persist across navigations
// within the app by design) - so a server-passed region prop can go stale
// the moment someone clicks a Link instead of hard-loading the page,
// silently sending "Discover" back to the national page from a region
// they've already navigated into. Parsing the live pathname sidesteps that
// entirely; it doesn't need DB validation because it only builds an href,
// never trusts the slugs for anything security- or data-sensitive - the
// destination page validates them itself.
export function SiteHeaderNav({
  ...
  myRegion: { stateSlug: string; areaSlug: string; areaName: string } | null;
  notificationEvents: NotificationBellEvent[];
  lastReadAt: string | null;
  adminNotifications: AdminNotificationBellItem[];
  ...
  // header's own "Discover" link is the site's main navigation, not part of
  // that panel, so it should always land on the full standalone page
  // instead.
  const headerDiscoverHref = regionSlugs
    ? discoverHref
    : myRegion
