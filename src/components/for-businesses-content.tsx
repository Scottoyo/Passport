...
// The full "For Businesses" marketing page, shared by the region-scoped
// route (src/app/[state]/[area]/for-businesses/page.tsx) and the top-level
// one (src/app/for-businesses/page.tsx) - the content itself is fully
// standardized/generic across every region already (no region-specific
// language or colors), so the only thing that ever differs between callers
// is where the "List Your Business"/"Register Your Business" links send
// someone: a region caller prefills that exact region, the top-level route
// sends them to the plain registration form to pick their own.
export function ForBusinessesContent({ registerHref }: { registerHref: string }) {
  return (
