...
// regardless. Shared by RegisterBusinessLink (desktop) and the mobile menu.
...
export function getRegisterBusinessHref(pathname: string): string {
  ...
  if (segments.length > 1 && segments[1] !== "businesses") {
    return `/register-business?state=${state}&region=${segments[1]}`;
  }
  return `/register-business?state=${state}`;
}
